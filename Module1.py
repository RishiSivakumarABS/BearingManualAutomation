...
with st.container():
    ...
    with col2:
        D = st.number_input("🏠 Outer Diameter (D) [mm]", min_value=d + 10, max_value=1200.0, value=d + 20)
...
if st.button("✅ Proceed to Design Calculations"):
    ...
    # Interpolate F
    lower_df = ira_df[(ira_df['inner_diameter'] <= d) & (ira_df['outer_diameter'] <= D)]
    upper_df = ira_df[(ira_df['inner_diameter'] >= d) & (ira_df['outer_diameter'] >= D)]

    if lower_df.empty or upper_df.empty:
        st.error("Not enough data to interpolate F. Try different d and D.")
    else:
        lower_match = lower_df.iloc[(lower_df['inner_diameter'] - d).abs().argsort()[:1]]
        upper_match = upper_df.iloc[(upper_df['inner_diameter'] - d).abs().argsort()[:1]]

        f_low = lower_match.iloc[0]['f']
        f_high = upper_match.iloc[0]['f']
        d_low = lower_match.iloc[0]['inner_diameter']
        d_high = upper_match.iloc[0]['inner_diameter']

        if d_high != d_low:
            F = f_low + ((f_high - f_low) / (d_high - d_low)) * (d - d_low)
        else:
            F = f_low  # fallback to same match
